...
const saveRow = async (row) => {
  try {
    if (row.isNew) {
      await axios.post(`/api/versions/parts/${props.partId}/parameters`, {
      ...
      await axios.put(`/api/versions/parameters/${row.id}`, {
        param_name: row.param_name,
        param_value: row.param_value,
        param_unit: row.param_unit
      })
...
const deleteParam = async (id) => {
  await axios.delete(`/api/versions/parameters/${id}`)
