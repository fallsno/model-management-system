<template>
  <div>
    <el-upload
      class="upload-demo"
      drag
      :action="`/api/versions/${versionId}/upload`"
      :headers="{}"
...
const fetchAttachments = async () => {
  const res = await axios.get(`/api/versions/${props.versionId}/attachments`)
...
const downloadFile = (id) => {
  window.open(`/api/versions/attachments/${id}/download`)
}

const deleteFile = async (id) => {
  await axios.delete(`/api/versions/attachments/${id}`)
